...
- `CMakeLists.txt`, `tools/build.ps1`, `tools/build.sh`, `tools/build_matrix.ps1`: firmware version `0.3.5`.
...
- `docs/firmware/*/*.factory.bin`, `docs/release-fingerprints-v0.3.5.sha256`: rebuilt release artifacts and hashes.
...
```powershell
.\tools\build_matrix.ps1 -Targets esp32c6 -Version 0.3.5
```
...
- Replace: seven `docs/firmware/*/ios-ancs-*-v0.3.3.factory.bin` files with `v0.3.5` builds
- Replace: `docs/release-fingerprints-v0.3.3.sha256` with `docs/release-fingerprints-v0.3.5.sha256`

- [ ] **Step 1: Update release tests first and verify RED**

Set firmware `VERSION = "0.3.5"`, rename test names from `v033` to `v034`, change installer/docs expectations to `0.3.5`, and change companion manifest expectations to `0.6.3`. Add README assertions that native ANCS Display Name is primary and static mapping is fallback. Run tool and manifest contract tests; expect failures against current `0.3.3`/`0.6.2` files.

- [ ] **Step 2: Update textual version anchors and guidance**

Set firmware defaults/project version to `0.3.5` and integration manifest to `0.6.3`. Update both READMEs and installer surfaces to explain: first notification from an app may incur one native lookup; subsequent notifications use a session cache; app lookup failure falls back without losing notification details; HACS installs the companion only.
...
```powershell
.\tools\build_matrix.ps1 -Version 0.3.5 -Jobs 6
```
...
Trailers: `Constraint: Firmware 0.3.5 and companion 0.6.3 must remain independently identifiable`, `Tested: full pytest suites, seven-target builds, compileall, release integrity`, `Not-tested: physical iPhone notification until deployment step`.
