...
triggers:
  - trigger: state
    entity_id: sensor.ios_ancs_c6_ab12_last_notification
conditions:
  - condition: template
    value_template: >-
...
Generate one new iOS app notification, prove one MQTT payload and one `notify.mobile_app_example_phone` result, then prove the `[C6→HA]` reflected ANCS event produces no MQTT publish.
