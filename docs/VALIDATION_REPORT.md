...
- The original office 2.4 GHz SSID remained marginal at the test location, so
  ...
  `SPARKPLUS14_4F` configuration was restored with MQTT secrets preserved, and
...
- After validation, the Windows hotspot was stopped and the device was restored
  to its original `SPARKPLUS14_4F` Wi-Fi configuration with MQTT secrets
  ...
  setup AP remained available. AX1800 stayed connected to `SPARKPLUS14_5F` and
...
- The flashed device booted, automatically exposed
  `IOS-ANCS-SETUP-572B20`, and served `/api/status`. The live portal contains
...
- With DTR/RTS fixed before opening COM9, holding the board's BOOT GPIO9 low
  ...
  `IOS-ANCS-C6-2B20` at Bluetooth address `40:4C:CA:57:2B:22` and RSSI
  `-63 dBm`.
...
- The broker retained the v0.3.0 button Discovery config with unique ID
  `ios_ancs_c6_2b20_enroll`, command topic
  `ios-ancs/c6-2b20/command/enroll`, exact payload `ENROLL`, QoS 1, and
  ...
  retained.
- Home Assistant registered `button.ios_ancs_c6_2b20_enroll` as an enabled
...
- Calling Home Assistant's real `button.press` service emitted exactly one
  ...
  an active BLE scan immediately found `IOS-ANCS-C6-2B20` at
