...
- `production-serial-after-review.log`: COM9 boot started STA for `SPARKPLUS14_5F`, started setup AP `IOS-ANCS-SETUP-572B20` at `192.168.4.1`, loaded BLE bond count `1`, and entered `state=advertising`.
- `windows-wlan-before-connect-after-review.txt`: DAISO was the setup adapter, GUID `{FEB7E51C-7AB8-4993-B725-E4E8058764FD}`, IP `192.168.4.2/24`; AX1800 was inspected read-only and stayed connected to `SPARKPLUS14_5F`.
...
- `artifacts/task5-final-dupfix/windows-wlan-readonly-dupfix.txt`: DAISO remained on the setup AP with `192.168.4.2`; AX1800 remained on `SPARKPLUS14_5F`; no portal config POST was performed in the dupfix pass.
