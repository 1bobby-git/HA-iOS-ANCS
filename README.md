...
이 파일을 Home Assistant automation YAML에 복사하거나 automation package에서 include할 수 있습니다. 자동화는 MQTT Discovery의 latest notification sensor 상태 변화를 트리거로 사용하고, incomplete 또는 `pre_existing` payload를 무시하며, `unavailable`에서 복구된 전환을 거부해 오래된 `relay_id`가 재전송되지 않게 합니다. 기본 예시는 `notify.mobile_app_1bobby`로 전송하고 모바일 알림 제목에 `[C6묶A]`를 붙입니다.
