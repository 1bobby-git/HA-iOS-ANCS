...
알려진 기준 보드의 base MAC suffix는 `572B20`입니다. 이 보드는 `IOS-ANCS-SETUP-572B20` AP와 `ancs-572b20` 비밀번호를 사용합니다. 다른 보드는 SSID에 대문자 MAC suffix를 쓰고, 설정 비밀번호에는 소문자 형식 `ancs-<lowercase_suffix>`를 씁니다. 인프라 Wi-Fi 비밀번호는 case-sensitive이며 입력한 그대로 저장됩니다.
...
이 파일을 Home Assistant automation YAML에 복사하거나 automation package에서 include할 수 있습니다. 자동화는 MQTT Discovery의 last-notification sensor 상태 변화를 트리거로 사용하고, incomplete 또는 `pre_existing` payload를 무시하며, `unavailable`에서 복구된 전환을 거부해 오래된 `relay_id`가 재전송되지 않게 합니다. 파일 안의 `notify.mobile_app_1bobby`와 `sensor.ios_ancs_c6_2b20_ios_ancs_c6_2b20_last_notification`은 저장소 소유자 환경 예시입니다. 자동화를 켜기 전에 `service:`를 `notify.mobile_app_<your_device>`로 바꾸고, trigger `entity_id:`를 본인 Home Assistant에서 발견된 last-notification sensor로 바꿔야 합니다. 예시 모바일 알림 제목 marker는 `[C6→HA]`입니다.
